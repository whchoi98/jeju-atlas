...
Claude Code에서 `ultracode`를 유지할 때는 요청 끝에 다음 문구를 포함합니다.
03–08장 카드에는 이미 포함되어 있습니다.

```text
Claude Code 요청: 검증 워크플로 없이 진행
```

별도 검증 워크플로를 생략하는 요청이며, 교재와 배포기의 필수 검사 및 완료 확인은 유지합니다.
...
Claude Code로 실행할 때 현재 세션 설정과 요청 범위의 필수 확인은 유지하세요.
Claude Code 요청: 검증 워크플로 없이 진행
```
