...
사전 구성, 01장 키 입력과 02장 확인 후에는 03–08장 하단 프롬프트를 한 장씩 사용합니다.\
본문 명령으로 완료한 작업은 프롬프트에서 반복하지 않고 남은 단계만 이어 갑니다.\
...
- [오류별 재개와 완료 상태 조회](jeju-atlas-install/references/recovery.md)
- [사용자용 설치 프롬프트와 복사 안내](installation-prompts.md)

다른 EC2에는 이 파일 하나가 아니라 `jeju-atlas-install/` 폴더 전체를 복사합니다.
기본 JejuGuide와 심화 Guide는 처음 입력한 같은 Bedrock API 키를 사용합니다.\
새 셸 활성화, Docker 권한과 계획·적용·상태 조회는 정식 패키지의 최신 절차를 따릅니다.
