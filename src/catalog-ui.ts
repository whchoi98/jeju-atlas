...
export class CatalogUI {
  ...
  constructor(root: HTMLElement, detailRoot: HTMLElement, options: CatalogOptions) {
    ...
    root.innerHTML = `
      ...
      <div class="catalog-filters"><label><span class="sr-only">카탈로그 분류</span><select id="catalog-category"><option value="">모든 분류</option></select></label><label class="catalog-map-switch"><input id="catalog-map-toggle" type="checkbox" checked>지도에 표시</label></div>
    ...
    root.querySelector('#catalog-map-toggle')!.addEventListener('change', (event) => options.onVisibility((event.target as HTMLInputElement).checked));
  ...
  onMapReady(): void {
    this.mapReady = true;
    this.options.onVisibility(this.root.querySelector<HTMLInputElement>('#catalog-map-toggle')!.checked);
    void this.loadPoints();
  }
  ...
  onMapMove(): void {
    ...
    this.moveTimer = setTimeout(() => {
      void this.loadPoints();
      if (this.mode === 'nearby') { this.offset = 0; void this.search(); }
    }, 600);
  }
  ...
  private async loadStatus(): Promise<void> {
    try {
      ...
      select.innerHTML = '<option value="">모든 분류</option>' + status.categories.map((category) =>
  ...
  private viewItems(): CatalogPlace[] {
    ...
    return this.points.features.filter((feature) => (!query || feature.properties.name.toLocaleLowerCase().includes(query))
  ...
  private renderList(): void {
    ...
    this.root.querySelector('#catalog-list')!.innerHTML = this.items.map((place) => `
      <button class="catalog-card ${place.id === this.detailId ? 'is-selected' : ''}" data-catalog-id="${html(place.id)}" aria-pressed="${place.id === this.detailId}">
        <span class="catalog-category-dot catalog-category-dot--${categoryColor(place.category)}">${icon(['food', 'restaurant', 'cafe', '맛집', '카페'].includes(place.category) ? 'pin' : 'globe')}</span>
  ...
  async loadPoints(): Promise<void> {
    ...
    if (bbox[0] >= bbox[2] || bbox[1] >= bbox[3]) {
      ...
      this.options.onPoints(this.points);
  ...
  private renderDetail(place: PlaceDetail): void {
    ...
    const facilities: Record<string, string> = { parking: '주차', wheelchair: '휠체어', kid_friendly: '아이 동반', pet: '반려동물', restroom: '화장실', wifi: '와이파이', outdoor_seating: '야외 좌석', credit_card: '카드 결제', reservation: '예약' };
    const facilityValue: Record<string, string> = { yes: '있음', no: '없음', limited: '제한적', unknown: noData };
    ...
    this.detailRoot.innerHTML = `
      ...
      <div class="detail-content">
        ...
        <section class="detail-section"><h3>요일별 이용시간</h3><table class="hours-table"><caption>제공된 요일별 이용시간 · 월요일 기준</caption><tbody>${days.map((day, index) => {
        ...
        }).join('')}</tbody></table><p class="micro-note">요일별 시간 출처: ${html(sourceName(place.hours_source))}</p><p class="detail-hours-raw">기본 시간 안내: ${html(place.hours || noData)}</p><p class="micro-note">기본 시간 출처: ${html(place.source_label || sourceName(place.source))} · 방문 전 제공처에 확인해 주세요.</p></section>
        ...
        <section class="detail-section"><h3>인허가 정보</h3><p id="business-registration">인허가 상태: ${business}</p><p class="micro-note">현재 시각의 영업 여부를 뜻하지 않습니다.</p><ul class="detail-sources">${this.sourceList(businessSources)}</ul></section>
