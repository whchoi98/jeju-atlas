...
def owner(repo, participant, project, assistant="claude"):
    if assistant not in {"claude", "codex", "kiro"}:
...
def check_owned(directory, expected):
    safe_path(directory)
    if directory.exists():
        if not directory.is_dir() or read_json(directory / ".owner.json") != expected:
            raise ValueError("Existing directory belongs to another setup; preserve it: " + str(directory))
...
def prepare(repo, participant, project, assistant="claude"):
    repo = check_source(repo)
    local, parent, destination, tools = paths(repo, participant, project)
    # Check every existing directory before writing anything. In particular do
    # not adopt the older Warmup or advanced app participant directories.
    ...
    for directory, expected in directories:
        if not directory.exists():
            ...
            write_new(directory / ".owner.json", json_text(expected))
    ...
    return {"activationPath": str(parent / "activate.sh"), "projectPath": str(destination),
            "toolchainPath": str(tools), "createdAwsResources": False, "installedTools": False}
...
def main():
    ...
    for action in ("prepare", "account", "doctor", "source", "tools"):
        ...
        if action == "prepare":
            command.add_argument("--assistant", choices=["codex", "kiro", "claude"], default="claude")
        if action == "doctor":
