# 13, 실습 자원 정리. Agentic AI 코딩 어시스턴트 카드
...
helper 명령은 `ATLAS_REPO`, CLI 자원 정리는 `ATLAS_CLI`로 다시 이동하세요.
경로 이동이나 활성화가 실패하면 그 셸의 후속 명령을 실행하지 마세요.

## 작업

cleanup을 먼저 계획 모드로 조회하고 소유 태그, 접두사, 계정, 삭제 순서와 보존 자원을 설명하세요.
공통 ID team01만으로 소유권을 판단하지 말고 현재 계정과 실제 바인딩도 대조하세요.
학습자가 정리 실행을 요청하고 소유 대상이 일치하는 경우에만 실행하세요.
CLI 입문과 Atlas 스택을 각각 정리하며 공유 VPC/NAT, 기존 인증서/DNS,
CDK bootstrap, 다른 프로젝트는 제외하세요.
S3 버전, Memory, 로그, SSM 잔존 여부를 기록하세요.
...
정리 전에 전체 테스트나 모델 응답을 다시 실행하지 마세요.
삭제 계획의 계정, 소유 대상 확인과 삭제 후 상태 조회는 유지합니다.
진행 중이거나 실패한 대상만 필요할 때 추가 조회하고 재생성하지 마세요.
삭제 미완료나 보존한 자원은 실제 상태와 다음 담당자를 기록합니다.
