...
Codex가 이미 설치된 실습 EC2에서 시작합니다. Codex 버전과 로그인 상태를 확인하되 재설치·인증 교체는 하지 마세요. Node 24, Python/uv, Docker Server, AWS CLI, @aws/agentcore 0.28.1을 확인하고 누락 도구만 준비하세요. 원본 프로젝트의 전역 도구를 바꾸지 말고 로컬 도구 경로를 사용하세요. 인증 파일·키를 읽지 마세요. PC는 HTML 교재를 읽는 용도입니다.

교재는 `$ATLAS_REPO/workshop/chapters/01-setup.md`입니다.
