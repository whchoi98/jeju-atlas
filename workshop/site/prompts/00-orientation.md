...
소스와 자원 매핑을 읽고 이 실습의 서비스 흐름을 설명하세요. CLI 입문 Runtime과 실제 Guide/Tools/Gateway/Memory, 공유 VPC/NAT와 실습 소유 자원, Codex 로그인과 Bedrock 모델 권한을 구분하세요. 파일이나 AWS를 변경하지 마세요.

교재는 `$ATLAS_REPO/workshop/chapters/00-orientation.md`입니다.
