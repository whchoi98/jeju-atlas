...
이 문서는 `workshop/IMPLEMENTATION_PLAN.md`와 저장소의 인프라·배포기·앱·Agent·라우터 소스를 연결한다. 기준은 2026-09-11에 읽은 소스이며, **참가자 계정의 배포 완료 보고서가 아니다.** 아래 검증은 해당 장에서 확보해야 할 증거다.

`infra/*.yaml` 10개의 **`Resources` 절에는 조건부 선언을 포함해 논리 리소스 94개, 리소스 유형 41개**가 있다. 예를 들어 `Parameters.VpcId.Type: AWS::EC2::VPC::Id`는 기존 VPC를 받는 입력 타입이다. 이를 VPC 생성 리소스로 세지 않는다. IAM 허용 ARN, 주석, 출력값, 과거 운영 보고서도 생성 선언과 구분한다.

파일 경로는 저장소 루트 기준이다. 표의 장 파일은 모두 `workshop/chapters/` 아래에 둔다. 장 배치는 구현 계획이며 작성·실행 완료를 뜻하지 않는다. 기존 `docs/`의 `.local/...` 자료는 비공개 과거 증거이므로 링크나 필수 다운로드로 사용하지 않는다.

**소유권 표기**

- **공유**: 사전에 제공받는 계정 기반·네트워크·DNS·인증서·서비스 접근 권한. 참가자 정리 대상이 아니다.
- **실습**: 격리된 작업 사본에서 팀 이름으로 생성하고 해당 팀의 실제 ARN/ID를 기록할 자원.
- **보존**: `DeletionPolicy: Retain`과 `UpdateReplacePolicy: Retain`이 선언된 실습 자원. 스택 삭제와 별도 정리가 필요하다.
- **외부/로컬**: 외부 데이터·서비스 또는 개발 환경·브라우저 자원. AWS 실습 스택의 소유 자원으로 세지 않는다.

`static.yaml`의 “shared assets”는 **한 팀의 여러 웹 릴리스가 공유하는 자산**이다. 기존 조직 VPC/NAT와 같은 공유 기반을 뜻하지 않는다.

## 장 연결표

| 장 파일 | 담당 범위 |
| --- | --- |
| `00-orientation.md` | 전체 경로, 기능 범위, 소유권과 증거 수준 |
| `01-setup.md` | Node·Python·uv·Docker·잠금 파일·브라우저 준비 |
| `02-aws-environment.md` | 팀 계정·프로필·이름, 기존 네트워크·권한·리전 |
| `03-codex.md` | 소스 탐색·변경 검토·검증을 위한 Codex 작업 |
| `04-agentcore-cli.md` | CLI가 관리하는 별도 입문 Runtime의 수명주기 |
| `05-foundation-and-data.md` | ECR, 데이터 저장소, 새 의존성·샘플 카탈로그 준비 |
| `06-atlas-agentcore.md` | 실제 Atlas Guide·Tools·Gateway·Memory·모델 설정 |
| `07-routing.md` | OSM·HGT 준비, Valhalla 이미지와 웹 이미지 쌍 |
| `08-web.md` | ECS·ALB·웹/BFF·지도·코스·AI 화면 |
| `09-https-edge.md` | 사용자 HTTPS, 원본 HTTPS, S3 OAC·캐시·WAF·엣지 함수 |
| `10-enrichment.md` | 제공처 권한, 공식 상세·사진 수집과 예약 갱신 |
| `11-operations.md` | 지표·로그·알람·SNS·사용량·복구 관측 |
| `12-validation.md` | 로컬 검사와 실제 계정·HTTP·브라우저·모델 검증 |
| `13-cleanup.md` | 의존성 역순 정리, 보존 자원·공유 기반 확인 |
...
| HMAC 세션·CSRF·SSE·분산 admission | `server/sessions.mjs`, `server/admission.mjs`, `server/guide.mjs`, `server/api.mjs` | `08-web.md`, `12-validation.md` | 서명 쿠키·`X-Atlas-CSRF`, actor/대화 바인딩, request ID, 16 KiB 본문·2,000자 질문, 8초 heartbeat·90초 BFF 제한 확인. 불명확한 실패를 자동 재과금 호출하지 않음 | 실습. 기본 하루 30회·actor 시간당 5회·전체 동시 2회는 금액 상한이 아님 |
...
| Codex CLI, AgentCore CLI 워크숍 도구 | `workshop/IMPLEMENTATION_PLAN.md` | `01-setup.md`, `03-codex.md`, `04-agentcore-cli.md` | 계획의 `@aws/agentcore` 0.28.1과 실제 설치 버전·스키마 대조. Codex 작업 인증과 배포 모델 설정 분리. 최종 명령은 해당 장·실제 도움말에서 확인 | 로컬 도구. 웹 런타임 의존성이 아님 |
