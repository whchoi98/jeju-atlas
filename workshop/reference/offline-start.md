...
처음에는 **[00장](../chapters/00-orientation.md)**에서 목표와 다음 단계를 확인합니다.\
준비가 필요한 환경은 [사전 구성](preconfiguration.md)을 마친 뒤 [01장의 활성화와 키 입력](../chapters/01-setup.md)으로 바로 이어갑니다.
...
사전 구성의 `start.sh`가 출력한 `activationPath`를 기록하고 새 Bash마다 같은 참가자 `activate.sh`를 현재 셸에 불러옵니다.\
...
이후 [01장](../chapters/01-setup.md)의 참가자 활성화와 키 입력으로 이어갑니다.\
