# 11, 관측, 보안, 운영 제어. Agentic AI 코딩 어시스턴트 카드
...
내 operations/data/edge 스택의 알람, dashboard, topic, heartbeat 구성을 확인하세요.
일일/시간/동시 한도, 기존 ECR 검사 상태, 로그 보관과 내용 capture 비활성화를 확인하세요.
SNS 수신은 학습자가 선택한 본인 채널의 확인 절차와 구분하세요.
계정 전체 설정이나 다른 프로젝트 IAM 정책을 임의 변경하지 마세요.

기본은 소유 스택의 읽기 전용 상태 확인과 요청한 구성 작업에 한정합니다.
전체 운영 검사, 부하 시험, 장애 주입, 알림 시험과 유료 모델 호출은 기본으로 생략합니다.
실제 오류, 운영, 보안 기능 변경 또는 명시적 요청이 있을 때만
해당 부분의 확인을 선택하세요. 구성 확인과 실제 알림 수신 성공을 구분합니다.
생략한 검사를 통과로 기록하지 마세요.

교재는 `$ATLAS_REPO/workshop/chapters/11-operations.md`입니다.
