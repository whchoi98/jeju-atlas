...
현재 폴더의 AGENTS.md와 .local/workshop-binding.json의 비밀값 없는 소유권 정보만 확인하세요. 독립 Git 작업 공간, 계정, 리전, Project/Stack 접두사를 설명하세요. lab.py run plan-bootstrap을 --execute 없이 실행하여 명령과 대상을 검토하세요. 이 카드 자체로 클라우드 변경을 실행하지 마세요.
