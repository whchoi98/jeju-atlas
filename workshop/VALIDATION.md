...
## 2026-09-25, Docker 그룹 적용과 소켓 권한 안내

새 EC2에서 Docker 설치 직후 `docker info`가 소켓 접근 권한 오류로 중단됐습니다.
참가자는 사용자 그룹 등록과 `newgrp docker`를 실행한 뒤 새 셸에서 확인하는
두 블록 안내로 정상 동작했다고 보고했습니다.

사전 구성과 설치 스킬을 패키지/데몬 준비, 그룹 등록과 새 셸 시작,
새 셸의 `docker info` 확인으로 구분했습니다. VSCode Server와 이미 실행한
코딩 어시스턴트의 기존 그룹 상속, 현재 셸과 등록된 그룹의 차이를 설명합니다.

워크숍 307개와 공개 교재 검사 2개를 통과했습니다.
수정한 문서의 Bash 블록 38개를 구문 검사하고, `newgrp`가 자기 블록의
마지막 명령이며 확인 명령과 분리되어 있는지 확인했습니다.
1366px/390px 브라우저에서 Docker 세 단계와 명령 4개 복사,
세 도구 탭, 인쇄와 오프라인 이동을 확인했습니다.

편집 호스트의 Docker 설치나 그룹을 변경한 검사는 아닙니다.
실제 EC2 해결 결과는 참가자 보고이며, 유지관리 검사는 교재와 복사 동작에 한정합니다.
증거는 `workshop/.local/publication-docker-groups-20260925T063848Z/`에 보관했습니다.

06:50 UTC에 공개 교재 파일 57개와 진입 경로 2개의 일치를 확인했습니다.
공개 사전 구성 페이지에서도 Docker 세 단계, 명령 4개 복사와
새 셸에서 확인하는 안내, 세 도구 탭과 오프라인 이동을 확인했습니다.
...
06:20 UTC에는 공개 교재 파일 57개와 진입 경로 2개의 일치를 확인했습니다.
공개 6장에서도 최초 키 재사용, 기존 Guide 전환 명령, 세 도구용 프롬프트
복사와 오프라인 이동을 확인했습니다.
...
05:24 UTC에 공개 교재 57개 파일과 진입 경로 2개의 일치를 확인했습니다.
공개 6장에서도 서울 리전 명령, 빌드 없는 재개 순서, 리전 입력 프롬프트 부재,
명령 및 공통 프롬프트 복사와 오프라인 이동을 확인했습니다.
