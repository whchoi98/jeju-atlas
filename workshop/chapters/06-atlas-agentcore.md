...
02장의 `lab.py model-region`으로 주최자가 확인한 값을 기록해야 합니다.
...
기존 GuideRole의 모델 권한도 자동 확장하지 않으므로 **Sonnet과 선택한 호출 리전에 맞는 Runtime 역할 검토를 주최자가 완료하기 전에는 agent-plan/apply를 진행하지 않습니다.**\
