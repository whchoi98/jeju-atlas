...
첫 Runtime 배포 후 선택하는 연동입니다.\
키가 없다면 이 장을 건너뛰고 시드 데이터를 계속 사용합니다.\
05~09장의 전체 앱이 준비된 참가자만 수집과 앱 반영을 진행합니다.

참가자의 Bash 터미널에서 필요한 키만 `.env`에 입력합니다.\
API 키를 AI 대화나 명령 인자에 넣지 않습니다.
...
```
`TOURAPI_SERVICE_KEY`와 `VISIT_JEJU_API_KEY` 중 준비한 키만 숨김 입력하고 나머지는 Enter로 건너뜁니다.\
이후 입력한 제공처만 게시합니다.

```bash
cd -- "${ATLAS_REPO:?먼저 01장의 activate.sh를 source하세요}" && {
python3 "$ATLAS_REPO/workshop/scripts/lab.py" set-secret --provider tourapi \
  --config "$ATLAS_CONFIG" --env-file "$ATLAS_CLI_PARENT/.env" --execute
python3 "$ATLAS_REPO/workshop/scripts/lab.py" set-secret --provider visitjeju \
  --config "$ATLAS_CONFIG" --env-file "$ATLAS_CLI_PARENT/.env" --execute
}
```
각 명령은 해당 키만 읽어 내 이름의 SSM SecureString으로 저장합니다.\
키가 비어 있으면 게시하지 않습니다.\
하나만 준비했다면 해당 명령만 실행합니다.
...
클러스터, Private Subnet, worker SG, Public IP 비활성화, 전용 task/execution role을 스택 출력, 생성 task definition과 대조합니다.

웹 task를 수집 작업 대신 실행하지 않습니다.\
...
Data 스택의 미디어 정책과 실제 Distribution이 맞지 않으면 S3를 공개하는 대신 08장의 데이터 정책 연결을 다시 확인합니다.

## 선택 실습, 카카오 장소 정보

카카오 Developers에서 실습 앱의 **카카오맵 사용 설정을 ON**으로 바꾸고 REST API 키를 준비합니다.\
키 종류를 혼동하지 않습니다.\
이 기능은 ECS의 REST 조회이므로 JavaScript, 네이티브 앱 키는 사용하지 않습니다.
...
```
참가자 앱의 `infra/production.json`에 `KakaoRestApiKeyParameter`를 `/jeju-atlas-lab-<참가자>/kakao-rest-api-key`로, `KakaoDailyLimit`를 `1000` 이하로 설정합니다.\
JSON에는 키 값이 아닌 내 SSM 경로만 넣습니다.\
진행자가 준비한 서로 다른 프로젝트의 키를 공유하지 않습니다.
...
```
계획에서 실행 역할의 `ssm:GetParameters`가 내 파라미터 ARN 한 개로 제한되고, 웹 컨테이너에는 ECS secrets로 전달되는지 확인합니다.\
브라우저 설정, JS, 로그에 키가 없어야 합니다.
...
Agentic AI 코딩 어시스턴트 프롬프트: [10, 공식 보강](../prompts/10-enrichment.md)
