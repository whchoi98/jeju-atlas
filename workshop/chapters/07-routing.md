...
입력 검증, OS 패치를 거쳐 불변 ECR 이미지로 게시하고
`.local/routing-image.json`에 digest, 자료 기준 시각을 기록합니다.
...
용량은 기존 실측 구성입니다. 모든 부하에 충분하다는 보장은 아니므로
동시 요청의 상태, 지연, OOM을 따로 봅니다. 예상 이동 시간은 실시간 교통 시간이 아닙니다.
...
AI CLI 프롬프트: [07, 라우팅](../prompts/07-routing.md)
