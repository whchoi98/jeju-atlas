...
여기부터는 120분 본 실습 이후의 선택 과정입니다.\
전체 Atlas를 배포할 별도 작업 사본을 만듭니다.\
...
이후 선택한 Agentic AI 코딩 어시스턴트는 이 app 폴더에서 실행합니다.\
PyYAML 오류로 binding 파일 없이 중단된 사본은 [사전 구성의 복구 절차](../reference/preconfiguration.md#pyyaml-누락으로-이미-사본-생성이-중단됐다면)에 따라 먼저 백업한 뒤 같은 참가자 설정으로 재준비합니다.
...
Public Access Block, 버전 관리, 암호화, 미디어 OAC를 만들지만 CloudFront 미디어 읽기 허용은 실제 Distribution을 연결할 때까지 없습니다.\
...
공식 확인 결과를 만들어 넣는 것이 아니라, 읽기 전용 worker가 첫 실행부터 정상 파일을 읽게 하는 준비입니다.\
이미 보강된 snapshot은 덮어쓰지 않습니다.
