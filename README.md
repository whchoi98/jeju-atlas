...
**검색·저장·길찾기·지도 조작 확장과 현재·누적 접속 표시를 운영 배포했습니다.**
현재 릴리스는 `release-20260912T105132Z` / `jeju-3d:25`입니다.
카카오맵에서 참고한 네 개의 탐색 탭, 최근 검색 후보, 즐겨찾기 분류·고정,
출발·도착 검색과 순서 뒤집기, 지도 지점 동작·주소 복사·장소 공유를 제공합니다.
이름 검색은 정확도순으로 보완했고, 공개 사이트에서 한림공원이 첫 결과로
표시되는 것과 해당 장소의 실제 도보·차량 경로를 확인했습니다.
`GuideLimitsEnabled: "false"`로 일일·시간당·전체 및 사용자별 동시 이용 한도를 해제했고,
같은 대화의 충돌과 요청 중복 실행은 별도로 방지합니다.
앱 하단의 `접속 N · 누적 M`은 최근 90초의 브라우저 세션과 집계 시작 이후 최초 접속 기록입니다.
새로고침·같은 세션의 여러 탭을 중복 집계하지 않으며 누적 값은 재배포 후에도 유지됩니다.
`UPDATE_COMPLETE`, desired/running 2/2, ALB 대상 2개 정상 및 최종 인프라·HTTP 검사 99개 통과를 확인했습니다.
...
검색·저장·경로·지도 통합 6개, 모바일 목록 6개, 지도·사이드바 14개 및 공개 PWA·워크숍 5개 검사가 통과했습니다.
앱 Node 683개·Python 197개가 통과했고, 선택 검사 7개는 별도 브라우저 검사와 구분해 기록했습니다.
성산 맛집과 아이 동반 원문 질문이 동시에 응답을 완료했습니다. 실제 카카오 식당과
TourAPI 주차 정보를 반환했고, 별도 주소·출처 요청도 정상 완료했습니다.
[검색·저장·길찾기 최신 배포 기록](docs/kakao-map-experience-release-2026-09-12.md) ·
[AI 한도·접속 표시 배포 기록](docs/ai-presence-release-2026-09-12.md) ·
[목록·사진 표시 배포 기록](docs/explore-usability-2026-09-12.md) ·
...
- 탐색·길찾기·저장·AI 가이드 탭, 최근 검색과 현재 결과의 키보드 검색 후보
- 저장 탭의 즐겨찾기 검색·분류, 최근 본 장소·검색 기록, 자주 쓰는 장소 5곳 고정
- 출발·도착 이름 검색, 도착 먼저 선택, 경유지·체류시간을 보존하는 전체 순서 뒤집기
- 지도 이동 후 명시적인 지역 검색, 현재 위치, 우클릭·터치 길게 누르기·키보드 지점 동작
- 목록 위치 강조, 주소·좌표 복사, 동일 장소 ID를 확인하는 장소 공유

이전 전용 Agent·경로 릴리스에 반영한 기능입니다. 당시 실제 네이티브 통합과 공개 브라우저 검사가 각각 8/8 통과했습니다.
...
카카오 키가 연결된 환경에서는 맛집·카페·숙소·주차장을 카카오 분류로 직접 조회합니다.
카테고리를 지정하지 않은 이름 검색도 카카오를 사용하며, 오름·해변·올레길 등 자연·관광 분류와
**검색 출처 → 기존 카탈로그**는 보존된 원자료를 사용합니다.
카카오 검색은 한 페이지 15곳, 검색별 최대 45곳입니다. 제공처의 검색 결과 수와 실제 조회 가능한 수를 구분합니다.

카카오 ID·주소·연락처·상세 링크를 직접 사용하고, 같은 장소임을 확인할 수 있는 기존 기록에만
공공 사진·이용 정보를 연결합니다. 기존 6,724건과 저장 코스의 ID를 삭제하거나 대량 카카오 DB로 교체하지 않습니다.
현재 조회 결과의 선택 토큰은 15분 동안 해당 세션에서만 유효하며, 여행·즐겨찾기에는 저장되지 않습니다.
저장한 카카오 장소를 다시 열면 같은 ID의 현재 정보를 조회하고, 확인되지 않으면 저장 정보임을 표시합니다.

| API | 용도 |
| --- | --- |
| `POST /api/kakao/search` | 카테고리·이름·지도 범위·주변 반경 조회 |
| `POST /api/kakao/detail` | 서명된 현재 선택 결과의 상세·공공 보강 |
| `POST /api/kakao/reopen` | 저장한 카카오 ID의 현재 정보 확인 |

위 API는 세션 쿠키와 요청 확인 토큰이 필요하며 캐시하지 않습니다.
기존 카카오 조회와 일일 1,000회 한도·세션별 제한·동시 실행 제한을 함께 사용합니다.
전체 검색 결과를 서버 DB나 PWA 캐시에 보관하지 않으며, 사용자가 선택한 최소 여행 스냅샷만 브라우저에 저장합니다.
`KAKAO_DISCOVERY_ENABLED=false` 환경 설정은 직접 탐색을 끄고 기존 카탈로그 방식으로 동작하게 합니다.

## AI와 저장

전용 Guide `JejuAtlas_Guide-2sFw9YBI8V`, Tools `JejuAtlas_Tools-Nh0YFIFC7c`,
HTTP Gateway와 Memory가 배포됐고 현재 ECS 앱에서 사용합니다.
브라우저는 AWS 자격 증명이나 런타임 ARN을 받지 않으며 서버가 IAM으로 호출합니다.
전용 Gateway 대상 경로는 `/JejuAtlasTools/invocations`입니다.

웹 AI의 상업 장소 추천도 BFF의 동일한 카카오 조회·사용량 제한을 사용합니다.
선택한 장소와 현재 조회 결과는 질문 원문과 분리된 구조화 데이터로 Guide에 전달합니다.
선택 토큰은 모델에 전달하지 않으며, 조회 원문은 장기 Memory에 저장하지 않습니다.
최신 데이터 지원 여부는 Guide 스트림의 버전 확인으로 검증하며, 이전 실행 환경은 새 대화를 안내합니다.
...
범위가 없는 아이 동반·실내 추천은 카탈로그에 기록된 태그와 분류에서 최대 3개 후보를 먼저 조회해 AI에 전달합니다. 한 곳을 요청하면 한 곳으로 제한하며, 태그 자체를 검증된 가족 적합성으로 설명하지 않습니다. 확인된 공공 상세 자료의 주차·화장실·접근성은 항목별 출처와 함께 전달합니다. 기존 AgentCore의 입력 한도 2,000자를 지키며 질문 원문을 유지합니다. 지역·주변·음식점·숙소 등 별도 조건이 있는 요청에는 이 기본 후보를 강제로 적용하지 않습니다. AI 검색 결과가 비면 카탈로그에서 조회한 참고 장소라는 점을 명시해 함께 표시하며, 통신 오류를 성공으로 바꾸지는 않습니다.
...
- 현재 운영은 `GuideLimitsEnabled: "false"`로 앱의 일일·시간당·전체 및 사용자별 동시 AI 한도를 해제했습니다. 같은 대화의 충돌과 요청 ID의 중복 실행 방지는 유지합니다.
- 대화 조정과 요청 상태를 `jeju-3d-guide-quota`에 원자적으로 기록합니다. 저장소 오류 시 AI 호출을 시작하지 않습니다. 한도 모드를 다시 켜면 별도 설정의 제한값이 적용됩니다.
...
`browser-guide-live-check.mjs`는 실제 AgentCore를 한 번 호출합니다. 원문의 한라산 질문, 올바른 지역의 식당, 생각 중·도구 표시, 글꼴 로드, 추천 지도와 후속 질문 입력을 확인합니다. 현재 앱 이용 한도는 해제되어 있으며 실제 모델 사용량은 발생합니다.
